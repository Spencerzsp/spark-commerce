...
object CreateMySqlPool {
  ...
  def apply(): GenericObjectPool[MySqlProxy] = {
    ...
    if (this.genericObjectPool == null) {
      this.synchronized {
        // 获取 MySQL 配置参数
        val jdbcUrl = ConfigurationManager.config.getString(MyConstant.JDBC_URL)
        val jdbcUser = ConfigurationManager.config.getString(MyConstant.JDBC_USER)
        val jdbcPassword = ConfigurationManager.config.getString(MyConstant.JDBC_PASSWORD)
